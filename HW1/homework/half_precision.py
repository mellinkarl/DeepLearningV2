...
class HalfLinear(torch.nn.Linear):
    def __init__(
        ...
        self.weight = torch.nn.Parameter(
            torch.empty(out_features, in_features, dtype=torch.float16), requires_grad=False)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        return torch.nn.functional.linear(x, self.weight, self.bias).to(torch.float32)
